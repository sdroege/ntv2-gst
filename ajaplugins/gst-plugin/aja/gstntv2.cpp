...
AJAStatus
NTV2GstAV::InitAudio (uint32_t * numAudioChannels)
{
  AJAStatus status (AJA_STATUS_SUCCESS);
...
AJAStatus NTV2GstAV::SetupAudio (void)
{
  ...
  if (mMultiStream &&::NTV2DeviceGetNumAudioStreams (mDeviceID) > 1
      && UWord (mInputChannel) <::NTV2DeviceGetNumAudioStreams (mDeviceID))
...
{
  mVideoBufferSize =
      GetVideoActiveSize (mVideoFormat, mPixelFormat, false, false);
