...
static void
gst_aja_sink_combiner_finalize (GObject * object)
{
...
gst_aja_sink_combiner_aggregate (GstAggregator * aggregator, gboolean timeout)
{
  ...
  // TODO: Make sure memory is stored in AJA memory

  if (audio_buffer) {
    gst_buffer_add_aja_sink_audio_meta (video_buffer, audio_buffer);
...
static gboolean
gst_aja_sink_combiner_sink_event (GstAggregator * aggregator,
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_CAPS:{
      ...
      if (agg_pad == GST_AGGREGATOR_PAD_CAST (self->audio_sinkpad)) {
        if (self->audio_caps && !gst_caps_can_intersect (self->audio_caps, caps)) {
        ...
        if (self->video_caps && !gst_caps_can_intersect (self->video_caps, caps)) {
...
static gboolean
gst_aja_sink_combiner_sink_query (GstAggregator * aggregator,
    GstAggregatorPad * agg_pad, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    // TODO: ALLOCATION: propose AJA buffer pools
    default:
      break;
  }
...
static gboolean
gst_aja_sink_combiner_negotiate(GstAggregator * aggregator)
{
    return TRUE;
}

static gboolean
gst_aja_sink_combiner_stop (GstAggregator * aggregator)
{
  GstAjaSinkCombiner *self = GST_AJA_SINK_COMBINER (aggregator);
...
static void
gst_aja_sink_combiner_class_init (GstAjaSinkCombinerClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (gst_aja_sink_combiner_debug, "ajasinkcombiner",
      0, "AJA sink combiner");
}

static void
gst_aja_sink_combiner_init (GstAjaSinkCombiner * self)
{
